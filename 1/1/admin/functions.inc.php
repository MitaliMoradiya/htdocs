...
function get_safe_value($con, $str) {
    // Ensure $str is a string before calling trim() and mysqli_real_escape_string()
    if (!is_string($str)) {
        return ''; // Handle non-string input by returning an empty string or appropriate default value
    }
    $str = trim($str);
    return mysqli_real_escape_string($con, $str);
}
